# JARVIS M3P2 milestone gate — the demo script
...
sentence JARVIS said. The gate's output is a transcript plus your notes, not a
...
```
cd ~/jarvis && uv run python scripts/gate_preflight.py
```
...
```
cd ~/jarvis && uv run python scripts/gate_observer.py
```
...
**Start it before `jarvis`, not after.** Whatever is already on disk belongs to
...
log), do that first and *then* run `jarvis` — it sees `/health` answering and
...
**Say:** nothing yet — this beat is JARVIS asking *you*.

**Expect:** "I found what looks like `<name>` at `<path>`. Is that one of
yours, sir?", then your spoken "yes", then "Noted, sir."

**KNOWN BLOCKER — read this before you try.** Nothing in the running server
calls `Onboarding.refresh()` or `ask_next()`. The brain answers the repo
question (`handle_reply`) but no code path ever *asks* it, and there is no
HTTP route or voice phrase that starts discovery. So JARVIS will never open
this beat on its own.

If the preflight told you to seed the registry by hand, then:

- **record beat 1 as NOT DEMONSTRATED**, with the reason "discovery is
  unreachable from the running server — `Onboarding.ask_next()` has no
  caller";
- the registry rows you seeded are visible in the observer at startup, which
  is what beats 2 and 8 stand on. That is not the same thing as beat 1, and
...
> the README saying hello from JARVIS.
...
#N spawned  w1 <project> -> IDLE_AT_PROMPT
    ...
    branch   jarvis/<slug>-<stamp> @ <commit>
...
1. **Answer only after the readback finishes.** A "yes" now resolves only an
   approval JARVIS has actually read aloud. Answer early and you get
...
and JARVIS says "`<project>` is yours in the terminal, sir."
...
type in the terminal. JARVIS holds no SDK stream for that session any more —
...
`--no-access-log` — which is what `bin/jarvis` does. The preflight prints the
...
project. If it was already confirmed during earlier testing, JARVIS goes
...
3. Restart the server the same way you started it (with the access log if the
   preflight told you to), then `jarvis`.

**Expect:** JARVIS reports the interrupted worker out loud, and a ghost tile
...
3. **The worktrees stay.** Nothing removes them automatically — each one holds
   ...
   `git -C <origin repo> branch -D jarvis/<...>`. Leaving them means the ghosts
