...
"I think the hooks kept arriving" is not evidence. This tails what Marvin
...
THE THREE FILES, AND WHAT EACH BEAT LEAVES BEHIND
  ...
  state/server.log    uvicorn's output. `POST /hooks` lines are beat 7's
                      ...
                      NOTE: bin/marvin starts uvicorn with
...
BEAT 7 GETS TWO LANES, because it is the beat with no automated proof:
  ...
  B. fleet records for a worker whose state is DETACHED. After the handoff,
     Marvin holds no SDK stream for that session at all (fleet.handoff closes
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--registry", default=str(REPO_ROOT / "config" /
                                              "projects.json"))
    ap.add_argument("--launcher", default=str(REPO_ROOT / "bin" / "marvin"))
    ...
    started = time.time()
    header = [
        "# Marvin M3P2 milestone gate — observer transcript",
        "",
    ...
    if access_log_disabled(launcher_text):
        emit("gate", "NOTE: bin/marvin starts uvicorn with --no-access-log, so "
