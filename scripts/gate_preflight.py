...
Run:   cd ~/marvin && uv run python scripts/gate_preflight.py
...
def check_proxy_env(env: dict) -> Check:
    ...
    if problem:
        return Check(
            ...
            "in ~/marvin/.env set HTTPS_PROXY=http://127.0.0.1:7890, "
...
def check_voice(env: dict) -> list[Check]:
    ...
    if env.get("DEEPGRAM_API_KEY"):
        ...
        out.append(Check(
            ...
            "add DEEPGRAM_API_KEY=<key> to ~/marvin/.env (it is currently "
    ...
    if forced == "say":
        ...
        out.append(Check(
            ...
            "add ELEVENLABS_API_KEY and ELEVENLABS_VOICE_ID to ~/marvin/.env "
...
def check_state_dir(state: Path) -> list[Check]:
    ...
    if not state.is_dir():
        out.append(Check(
            ...
            "mkdir -p ~/marvin/state  (the server creates it at boot, but the "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if env_path.exists():
        ...
        report.add(".env", ERROR, f"{env_path} is missing — bin/marvin sources "
                   ...
                   "create ~/marvin/.env with HTTPS_PROXY, HTTP_PROXY, "
    ...
    if endpoint is None:
        report.add("proxy reachable", ERROR,
                   ...
                   "probe",
                   "set HTTPS_PROXY=http://127.0.0.1:7890 in ~/marvin/.env")
    else:
        host, port = endpoint
    ...
    print("      cd ~/marvin && uv run python scripts/gate_observer.py")
